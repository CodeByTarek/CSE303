...
lasso = Lasso(fit_intercept=False,normalize=True,positive=True)

# For Fitting the model...
lasso.fit(X,y)             # X and y is from train Dataset...

#%%
# Prediction Value..
y_pred=lasso.predict(X)   #Prediction for X..here X is from Train Dataset...
...
# changing parameter
lasso = Lasso(fit_intercept=True,normalize=True,tol=1e-10)
...
y_pred_test=lasso.predict(test_data)   #Prediction for X..here X is from Test Dataset...
...
submission.to_csv('Sample Submission For Model 3.csv',index=False)
